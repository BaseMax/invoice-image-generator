...
function fagd($str,$z="",$method='normal'){
    ...
    if ($ar === null || empty($ar)) return $output;
    ...
    for ($i=0; $i<$str_len; $i++){
        ...
        if (isset($ar[0][$i+1]) && in_array($ar[0][$i+1],$ignorelist)){
            if (isset($ar[0][$i+2])) $str_next=$ar[0][$i+2];

            if ($i == 2) $str_back=$ar[0][$i-2];
            if ($i != 2) $str_back=$ar[0][$i-1];
        }
        else if ($i > 0 && isset($ar[0][$i-1]) && !in_array($ar[0][$i-1],$ignorelist)){
            if (isset($ar[0][$i+1])) $str_next=$ar[0][$i+1];
			
            if (isset($ar[0][$i-1])) $str_back=$ar[0][$i-1];
        }
        else {
        ...
        if (!in_array($str1,$ignorelist)){
            if (array_key_exists($str1,$p_chars)){
                if (!$str_back or $str_back==" " or !array_key_exists($str_back,$p_chars)){
                    if (!array_key_exists($str_back,$p_chars) and !array_key_exists($str_next,$p_chars)) {
                        ...
                        if (isset($output)) {
                            ...
                            $output = $p_chars[$str1][2];
                        }
                    }
                    continue;
                }
                else if (array_key_exists($str_next,$p_chars) and array_key_exists($str_back,$p_chars)){
                ...
                else if (array_key_exists($str_back,$p_chars) and !array_key_exists($str_next,$p_chars)){
                ...
                if (in_array($str1,$number)){
                    if (isset($num)) $num .= $str1;
					else $num = $str1;
                    $str1="";
                }
                if (!in_array($str_next,$number)){
                    if (isset($num)) {
                        if (isset($str1)) $str1.=$num;
                        else $str1 = $num;
                    }
                    $num="";
                }
				if (isset($output)) $output = $str1 . $output;
				else $output = $str1;
            }
            else {
